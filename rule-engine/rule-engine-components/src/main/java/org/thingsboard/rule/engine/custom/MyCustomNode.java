...
@Slf4j
@RuleNode(
        type = ComponentType.NEXAMACHINE,
        name = "NexaTel",
        relationTypes = {TbNodeConnectionType.TRUE, TbNodeConnectionType.FALSE},
        configClazz = MyCustomeNodeConfiguration.class,
        ...
        configDirective = "tbMyCustomNodeConfig")
public class MyCustomNode implements TbNode {

    private MyCustomeNodeConfiguration config;
    private ScriptEngine scriptEngine;

    @Override
    public void init(TbContext ctx, TbNodeConfiguration configuration) throws TbNodeException {
        this.config = TbNodeUtils.convert(configuration, MyCustomeNodeConfiguration.class);
